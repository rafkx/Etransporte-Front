...
export class VeiculoService {
  ...
  public downloadFile(fileName: string) {
    return this.http.get(
      `http://localhost:3000/files-veiculo/download/${fileName}`,
  ...
  public removeFile(fileName: string) {
    return this.http
      .delete(`http://localhost:3000/files-veiculo/${fileName}`)
      .pipe(first());
  }
  ...
  public addPhoto(id: string, photo: File) {
    ...
    return this.http.post(
      `http://localhost:3000/veiculo/photo/${id}`,
      formData2
    );
  }

  public getPhoto(id: string): Observable<Blob> {
    return this.http.get(`http://localhost:3000/veiculo/photo/${id}`, {
      responseType: 'blob',
    });
  }
  ...
  public autorizarVeiculo(autorizacao: AutorizacaoVeiculo) {
    ...
    return this.http
      .post('http://localhost:3000/veiculo/autorizacao', autorizacao)
      .pipe(first());
  }

  public getAutorizacoes(): Observable<any> {
    return this.http
      .get('http://localhost:3000/veiculo/autorizacao')
      .pipe(first());
  }
  ...
  private create(veiculo: Partial<Veiculo>) {
    return this.http
      .post<Veiculo>('http://localhost:3000/veiculo', veiculo)
      .pipe(tap((res: Veiculo) => (this.veiculo = res.id)));
  }

  private update(veiculo: Partial<Veiculo>) {
    return this.http
      .patch<Veiculo>(`http://localhost:3000/veiculo/${veiculo.id}`, veiculo)
      .pipe(first());
  }

  public remove(id: string) {
    return this.http
      .delete(`http://localhost:3000/veiculo/${id}`)
      .pipe(first());
  }
}
